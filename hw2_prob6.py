...
for i in range(len(sales_data)):
    current_month = sales_data[i]["month"]
    previous = sum(sales_data[i - 1]["sales"])
    current = sum(sales_data[i]["sales"])
    sales[current_month] = current

    if current <= previous:
        decreasing_months.append(current_month)
